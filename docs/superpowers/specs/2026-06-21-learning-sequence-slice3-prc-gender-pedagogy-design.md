...
| C2 | **Reconciliación contra el oráculo** | Pura: `reconcile_rule(rule, oracle_gender, lemma) -> GenderRuleDetail`. **Solo se invoca con un `rule` no-nulo**; el guard sin-sufijo→`detail=null` vive en el call site (`record_gender_attempt`), no dentro de `reconcile_rule` (por eso su retorno nunca es `None`). **La compuerta de mostrabilidad es `agreement || is_exception`** (Caso A: la regla coincide con el oráculo; Caso C: excepción curada) — el oráculo siempre gana la pantalla, y una regla que discrepa sin estar curada (Caso B) se suprime. `suffix_class` (hard\|tendency) solo le da sabor a la copy del Caso A, no controla el flujo. La proyección `GenderRuleDetail` (6 llaves) → `GenderRuleOut` (4 llaves) se construye también en el call site. Política de casos en §4. |
