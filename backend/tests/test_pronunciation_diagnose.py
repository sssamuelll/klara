...
def _phonemes():
    return [
        PhonemeScore(phoneme="aʊ", accuracy_score=90.0),
        PhonemeScore(phoneme="uː", accuracy_score=38.0),
    ]
...
async def test_empty_phonemes_skips_llm(db_session):
    llm = FakeLLM('{"tip": "x"}')
    out = await generate_diagnosis(
        llm, db_session, word="Haus", phonemes=[], target_language="de", native_language="es"
    )
...
async def test_cache_miss_calls_llm_and_inserts_row(db_session):
    ...
    assert llm.calls == 1
    count = (
        await db_session.execute(select(func.count()).select_from(PronunciationDiagnosis))
    ).scalar()
    assert count == 1
...
async def test_diagnose_requires_auth(client, app_settings):
    r = await client.post(
        ...
        json={
            "language": "de",
            "word": "Haus",
            "phonemes": [{"phoneme": "aʊ", "accuracy_score": 40.0}],
        },
    )
...
async def test_diagnose_uses_user_native_language(client, app_settings, seed_invite, monkeypatch):
    ...
    r = await client.post(
        ...
        json={
            ...
            "native_language": "INVALID-CLIENT-VALUE",
        },  # must be ignored
    )
...
@pytest.mark.asyncio
async def test_diagnose_service_error_is_best_effort(
